...
function ProductList({ onHomeClick }) {
    ...
    const [addedToCart, setAddedToCart] = useState({}); // Tracks products added to cart { [name]: true }
    const dispatch = useDispatch();
    const cart = useSelector(state => state.cart.items);

    // Keep addedToCart in sync with global cart so buttons re-enable when items removed
    useEffect(() => {
        const map = {};
        cart.forEach(item => {
            if (item && item.name) map[item.name] = true;
        });
        setAddedToCart(map);
    }, [cart]);
    ...
    const handleAddToCart = (plant) => {
        // Dispatch selected plant to Redux cart slice
        dispatch(addItem(plant));
        // Update local state to reflect addition (use plant name as key)
            ...
            <div className="navbar" style={styleObj}>
                ...
                <div style={styleObjUl}>
                    ...
                    <div style={{ position: 'relative' }}>
                        ...
                        <span className="cart_quantity_count">{cart.reduce((s, i) => s + (i.quantity || 1), 0)}</span>
                    </div>
                </div>
            </div>
            {!showCart ? (
                            ...
                            <div className="product-list">
                                {category.plants.map((plant) => (
                                    <div className="product-card" key={`${category.category}-${plant.name}`}>
                                        ...
                                        <button
                                            className={`product-button add-to-cart ${addedToCart[plant.name] ? 'added-to-cart' : ''}`}
